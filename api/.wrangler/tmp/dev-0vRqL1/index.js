...
// .wrangler/tmp/bundle-HFruAy/strip-cf-connecting-ip-header.js
...
var TABLAS = {
  ...
  programacionCultivos: {
    ...
    columnas: [
      ...
      "fechaRealCosecha",
      "fechafinal",
      "numeroPlantasSembradas",
      "numeroPlantasCosechadas",
      "lote",
      "cama",
      "tipoAbono",
      "cantidadAbono0",
...
var columnasEscribibles = /* @__PURE__ */ __name((tabla) => {
  const t = TABLAS[tabla];
  return t.columnas.filter((c) => !t.generadas.includes(c));
}, "columnasEscribibles");
var columnasAlta = /* @__PURE__ */ __name((tabla) => {
  const t = TABLAS[tabla];
  return columnasEscribibles(tabla).filter((c) => !(t.autonumerica && c === t.pk));
}, "columnasAlta");

// src/queries/consultas-accion.mjs
var CONSTANTES = {
  PRODUCTO_ABONO_LIQUIDO: 2,
  // multimineral
  PRODUCTO_ABONO_SOLIDO: 3,
  // abono solido 1
  PRODUCTO_ABONO_LIQUIDO_2: 4,
  // multimineral 2
  PRODUCTO_BASILUS: 5,
  // Basilus
  PRODUCTO_MANO_DE_OBRA: 999,
  // fila de referencia creada en la migracion
  JORNAL_HORA: 8807,
  // pesos/hora en las consultas IngresoCostos*
  COSTO_MINUTO: 1.46,
  // pesos/minuto en las labores de campo
...
var C = CONSTANTES;
var COLS_ACTIVIDADES = "codigoSistema, codsemilla, fechaSiembra, Actividad, semanaAbono, cantidadAbono, lote, cama, numeroPlantas, detalle, unidad, costo";
var COLS_COSTOS = "concepto, detalle, fecha, unidad, valorUnitario, programacionCultivoCodCultivo, cantidad, producto";
var sem = /* @__PURE__ */ __name((dias) => dias === 0 ? `CAST(strftime('%U', pc.fechasiembra) AS INTEGER) + 1` : `CAST(strftime('%U', date(pc.fechasiembra, '+${dias} day')) AS INTEGER) + 1`, "sem");
...
__name(actividadDesdeProducto, "actividadDesdeProducto");
...
__name(actividadDeLabor, "actividadDeLabor");
...
__name(costoManoDeObra, "costoManoDeObra");
var CONSULTAS_ACCION = [
  // ------------------------------------------------- abono liquido (4)
  {
    ...
    sql: actividadDesdeProducto({
      actividad: "AbonoLiquido",
      dias: 8,
      campoCantidad: "abonoLiquido",
      productoId: C.PRODUCTO_ABONO_LIQUIDO
    }),
    origenAccess: 'INSERT INTO actividades (...) SELECT ... "AbonoLiquido", Format$(([fechasiembra]+8),"ww",0,0), infoSemilla.abonoLiquido, ... WHERE productos.id=2'
  },
  {
  ...
  },
  {
    ...
    sql: actividadDesdeProducto({
      ...
      dias: 50,
      campoCantidad: "abonoLiquido",
      productoId: C.PRODUCTO_ABONO_LIQUIDO_2,
      filtro: "s.ciclo > 50"
    }),
  ...
  // -------------------------------------------------- abono solido (3)
  {
    ...
    sql: actividadDesdeProducto({
      actividad: "AbonoSolido",
      dias: 25,
      campoCantidad: "abonoPrimera",
  ...
  },
  {
    ...
    sql: actividadDesdeProducto({
      actividad: "AbonoSolido",
      dias: 50,
      campoCantidad: "abonoSegunda",
      productoId: C.PRODUCTO_ABONO_SOLIDO,
      filtro: "s.abonoSegunda > 0"
    }),
  ...
  },
  {
    ...
    sql: actividadDesdeProducto({
      actividad: "AbonoSolido",
      dias: 75,
      campoCantidad: "abonoTercera",
      productoId: C.PRODUCTO_ABONO_SOLIDO,
      filtro: "s.abonoTercera > 0"
    }),
  ...
  // ------------------------------------------------ labores de campo (6)
  {
    ...
    sql: actividadDeLabor({
      actividad: "PreparacionTerreno",
      detalle: "PreparacionTerreno",
      dias: 0,
      minutos: C.MIN_PREPARACION_TERRENO
    }),
  ...
  },
  {
    ...
    sql: actividadDeLabor({
      actividad: "Deshierbe",
      detalle: "PrimerDeshierbe",
      dias: 25,
      minutos: C.MIN_DESHIERBE
    }),
  ...
  },
  {
    ...
    sql: actividadDesdeProducto({
      actividad: "ProteccionVegetal",
      dias: 0,
      campoCantidad: "abonoLiquido",
      productoId: C.PRODUCTO_BASILUS,
      filtro: "s.Aplicacion1 > 1"
    }),
...
ON CONFLICT DO NOTHING`,
    origenAccess: "(([infoSemilla]![abonoSiembra]+[abonoPrimera]+[abonoSegunda]+[abonoTercera])*[programacionCultivos]![numeroPlantasSembradas]) ... WHERE productos.id=3"
  },
  {
...
ON CONFLICT DO NOTHING`,
    origenAccess: "[abonoLiquido]*([ciclo]/15)*[numeroPlantasSembradas] ... WHERE productos.id=4"
  },
...
FROM productos p, infoSemilla s
     INNER JOIN programacionCultivos pc ON s.Id = pc.codSemilla
WHERE p.id = ${C.PRODUCTO_ABONO_SOLIDO}`,
    origenAccess: "(([abonoSiembra]+[abonoPrimera]+[abonoSegunda]+[abonoTercera])*[numeroPlantasSembradas]/1000) ... WHERE productos.id=3"
  }
];
var LOTES = {
  abonamiento: ["actualizarCostosAbonamiento", "actualizarCostosAbonamientoLiquido"],
  actividades: [
    "IngresoPreparacionTerreno",
    "IngresoSiembra",
    "IngresoAbonoSolido1Aplicacion",
    "IngresoAbonoSolido2Aplicacion",
    "IngresoAbonoSolido3Aplicacion",
    "IngresoAbonoLiquido1Aplicacion",
    "IngresoAbonoLiquido2Aplicacion",
    "IngresoAbonoLiquido3Aplicacion",
    "IngresoAbonoLiquido4Aplicacion",
    "IngresoPrimerDeshierbe",
    "IngresoSegundoDeshierbe",
    "IngresoProtecionBasilus",
    "IngresoProtecionBasilus2"
  ],
  costosManoDeObra: [
    "IngresoCostosPreparacionTerreno",
    "IngresoCostosSiembra",
    "IngresoCostosDeshierbe",
    "IngresoCostosCobertura"
  ],
  inventario: ["salidaAbono"]
};
var porNombre = /* @__PURE__ */ __name((nombre) => CONSULTAS_ACCION.find((c) => c.nombre === nombre), "porNombre");

// src/queries/vistas-parametrizadas.mjs
var SQL_PROGRAMACION_ABONAMIENTO = `
SELECT pc.codigosistema,
       pc.codSemilla,
       pc.fechasiembra,
...
// src/access-compat/fechas.mjs
var ZONA = "America/Bogota";
function hoyBogota(ahora = /* @__PURE__ */ new Date()) {
...
__name(hoyBogota, "hoyBogota");

// src/index.ts
var app = new Hono2();
app.use("/api/*", cors());
var noEncontrado = /* @__PURE__ */ __name((que) => ({ error: `No existe ${que}.` }), "noEncontrado");
...
app.get("/api/tablas/:tabla", async (c) => {
  ...
  if (!meta)
    return c.json(noEncontrado(`la tabla ${tabla}`), 404);
  const limite = Math.min(Number(c.req.query("limite") ?? 500), 2e3);
  const desde = Number(c.req.query("desde") ?? 0);
  const { results } = await c.env.DB.prepare(`SELECT * FROM ${tabla} ORDER BY ${meta.pk} LIMIT ?1 OFFSET ?2`).bind(limite, desde).all();
...
app.get("/api/tablas/:tabla/:id", async (c) => {
  ...
  if (!meta)
    return c.json(noEncontrado(`la tabla ${tabla}`), 404);
  const fila = await c.env.DB.prepare(`SELECT * FROM ${tabla} WHERE ${meta.pk} = ?1`).bind(c.req.param("id")).first();
...
app.post("/api/tablas/:tabla", async (c) => {
  ...
  if (cols.length === 0)
    return c.json({ error: "No se envio ningun campo que se pueda guardar." }, 400);
  const sql = `INSERT INTO ${tabla} (${cols.join(", ")}) VALUES (${cols.map((_, i) => `?${i + 1}`).join(", ")}) RETURNING *`;
...
app.put("/api/tablas/:tabla/:id", async (c) => {
  ...
  if (cols.length === 0)
    return c.json({ error: "No se envio ningun campo que se pueda modificar." }, 400);
  const sql = `UPDATE ${tabla} SET ${cols.map((x, i) => `${x} = ?${i + 1}`).join(", ")} WHERE ${meta.pk} = ?${cols.length + 1} RETURNING *`;
  const fila = await c.env.DB.prepare(sql).bind(...cols.map((x) => cuerpo[x] ?? null), c.req.param("id")).first();
  return fila ? c.json(fila) : c.json(noEncontrado("ese registro"), 404);
});
app.delete("/api/tablas/:tabla/:id", async (c) => {
  ...
  if (!meta)
    return c.json(noEncontrado(`la tabla ${tabla}`), 404);
  const r = await c.env.DB.prepare(`DELETE FROM ${tabla} WHERE ${meta.pk} = ?1`).bind(c.req.param("id")).run();
  return r.meta.changes > 0 ? c.json({ borradas: r.meta.changes }) : c.json(noEncontrado("ese registro"), 404);
});
app.get("/api/vistas", (c) => c.json(VISTAS));
app.get("/api/vistas/:nombre", async (c) => {
  const nombre = c.req.param("nombre");
  if (!VISTAS.includes(nombre))
    return c.json(noEncontrado(`la vista ${nombre}`), 404);
  const { results } = await c.env.DB.prepare(`SELECT * FROM "${nombre}"`).all();
  return c.json(results);
});
app.get("/api/informes/programacion-abonamiento", async (c) => {
  const { results } = await c.env.DB.prepare(SQL_PROGRAMACION_ABONAMIENTO).bind(hoyBogota()).all();
  return c.json(results);
});
app.get("/api/informes/trazabilidad", async (c) => {
  ...
  const { results } = await c.env.DB.prepare(SQL_PROGRAMACION_CULTIVO).bind(hoyBogota(), fechaInicial).all();
  return c.json(results);
});
...
async function ejecutarProceso(env, nombre) {
  const consulta = porNombre(nombre);
  if (!consulta)
    return null;
  const antes = await env.DB.prepare(`SELECT COUNT(*) AS n FROM ${consulta.destino}`).first();
  await env.DB.prepare(consulta.sql).run();
  const despues = await env.DB.prepare(`SELECT COUNT(*) AS n FROM ${consulta.destino}`).first();
  return {
    consulta: nombre,
...
__name(ejecutarProceso, "ejecutarProceso");
...
app.post("/api/procesos/lote/:lote", async (c) => {
  ...
  if (!nombres)
    return c.json(noEncontrado(`el lote ${lote}`), 404);
  const resultados = [];
  for (const n of nombres)
    resultados.push(await ejecutarProceso(c.env, n));
  return c.json({
    lote,
    resultados,
...
app.get("/api/adjuntos/:id/contenido", async (c) => {
  const fila = await c.env.DB.prepare("SELECT r2_key, mime, nombre_archivo FROM adjuntos WHERE id = ?1").bind(c.req.param("id")).first();
  if (!fila)
    return c.json(noEncontrado("ese adjunto"), 404);
  const objeto = await c.env.ADJUNTOS.get(fila.r2_key);
  if (!objeto)
    return c.json({ error: "El archivo no esta en el almacen." }, 404);
...
app.get("/api/adjuntos/:tabla/:registroId", async (c) => {
  const { results } = await c.env.DB.prepare("SELECT id, nombre_archivo, mime, bytes FROM adjuntos WHERE tabla = ?1 AND registro_id = ?2").bind(c.req.param("tabla"), c.req.param("registroId")).all();
  return c.json(results);
});
app.get("/api/cuarentena", async (c) => {
  const { results } = await c.env.DB.prepare("SELECT * FROM _cuarentena ORDER BY tabla_origen, regla, id").all();
  return c.json(results);
});
...
// .wrangler/tmp/bundle-HFruAy/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-HFruAy/middleware-loader.entry.ts
